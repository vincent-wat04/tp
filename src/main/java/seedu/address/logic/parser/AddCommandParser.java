...
=======
                ArgumentTokenizer.tokenize(args, PREFIX_NAME, PREFIX_PHONE, PREFIX_EMAIL,
                        PREFIX_ADDRESS, PREFIX_NEXT_MEETING, PREFIX_TAG);
...
=======
>>>>>>> 01549fbd (Add optional next meeting field support)
        String nextMeetingValue = argMultimap.getValue(PREFIX_NEXT_MEETING).orElse(NextMeeting.DEFAULT_VALUE);
        NextMeeting nextMeeting = ParserUtil.parseNextMeeting(nextMeetingValue);
        Set<Tag> tagList = ParserUtil.parseTags(argMultimap.getAllValues(PREFIX_TAG));

        Person person = new Person(name, phone, email, address, company, nextMeeting, tagList);
